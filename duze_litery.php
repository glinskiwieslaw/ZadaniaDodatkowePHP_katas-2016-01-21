...
function firstUC ($myString)
{
    $n = preg_match_all ('/[A-Z]/', $myString, $matches, PREG_OFFSET_CAPTURE);
    return $n ? $matches[0] : false;

}

function findCapitals($myString)
{
    $ucLetters = firstUC($myString);
    $results = array();
    if ($ucLetters) {

        for ($row = 0; $row < sizeof($ucLetters); $row++) {
            $results[$row] = $ucLetters[$row][1];
        }
        return $results;
    } else {
        return $results;
